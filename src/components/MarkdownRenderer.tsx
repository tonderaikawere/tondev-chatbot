...
function RichText({ text }: { text: string }) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('### ')) {
      flushList(i);
      renderedLines.push(
        <h4 key={i} className="text-base font-bold text-blue-900 mt-4 mb-2">
      ...
      flushList(i);
      renderedLines.push(
        <h3 key={i} className="text-lg font-bold text-blue-900 mt-5 mb-2 border-b border-blue-100 pb-1">
      ...
      flushList(i);
      renderedLines.push(
        <h2 key={i} className="text-xl font-bold text-blue-900 mt-6 mb-3">
    ...
    // Check for numbered lists
      ...
      renderedLines.push(
        <div key={i} className="flex items-start space-x-2 my-1">
          <span className="font-bold text-blue-800 flex-shrink-0">{line.match(/^\d+/)?.[0]}.</span>
          <p className="flex-1 text-gray-800">{parseInlineStyles(cleaned)}</p>
        </div>
      );
    }
    // Check for blockquotes
      ...
      flushList(i);
      renderedLines.push(
        <blockquote key={i} className="border-l-4 border-blue-500 pl-4 py-1 my-2 bg-blue-50/50 rounded-r text-gray-700 italic">
          {parseInlineStyles(line.slice(2))}
        </blockquote>
      );
    }
...
function parseInlineStyles(text: string): React.ReactNode[] {
  ...
  return tokens.map((token, index) => {
    if (token.startsWith('**') && token.endsWith('**')) {
      return <strong key={index} className="font-bold text-gray-900">{token.slice(2, -2)}</strong>;
    } else if (token.startsWith('*') && token.endsWith('*')) {
      return <em key={index} className="italic text-gray-800">{token.slice(1, -1)}</em>;
    } else if (token.startsWith('`') && token.endsWith('`')) {
      return (
        <code key={index} className="px-1.5 py-0.5 mx-0.5 rounded bg-blue-100/60 border border-blue-200/50 text-blue-900 font-mono text-xs sm:text-sm font-medium">
          {token.slice(1, -1)}
        </code>
      );
    } else {
